...
def make_fake_title(file_list: list, savedir: str, cfg_method: dict) -> None:
    ...
    if cfg_method['name'] in ['bow_title_category_select', 'bow_content_category_select']:
        preload_sim_argmax = json.load(open(f"{cfg_method['bow_dir']}/sim_argmax.json", 'r'))
...
def preprocess(file_list: list, cfg_method: dict) -> None:
    ...
    '''
    if cfg_method['name'] in ['bow_title_category_select', 'bow_content_category_select']:
        if not os.path.exists(os.path.join(cfg_method['bow_dir'], 'sim_argmax.json')):
            os.makedirs(cfg_method['bow_dir'], exist_ok=True)
            kwargs = {
